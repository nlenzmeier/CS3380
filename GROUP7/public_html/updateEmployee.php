...
-->
  ...
  if (isset($_POST['update'])) {
    ...
    $stmt = $mysqli->prepare("SELECT username, first_name, last_name, position FROM employee WHERE username = ?");
  ...
  if (isset($_POST['submit'])) {
    ...
    $stmt = $mysqli->prepare("UPDATE employee SET first_name = ?, last_name = ?, position = ? WHERE username = ?");
    $stmt->bind_param("ssis", $fname, $lname, $position, $username);
    if (!$stmt->execute()) {
      echo "Error: " . $stmt->error;
      return;
    }
...
<form action="updateEmployee.php" method="post">
  ...
  Position: <select name="position">
    <?php
      $positions = array("Admin", "Engineer", "Conductor");
      foreach ($positions as $index => $pos) {
        if ($index == $row['position']) {
          echo "<option selected='selected' value='$index'>$pos</option>";
        } else {
          echo "<option value='$index'>$pos</option>";
        }
      }
    ?>
  </select><br>
